...
/** Place a SPOT order on Nia-Hub (used by Swap). userId is injected server-side. */
export async function placeNiaOrder(o: {
  symbol: string;            // e.g. "BTC_USDT" (no engine prefix)
  side: 'BUY' | 'SELL';
  type: 'MARKET' | 'LIMIT';
  quantity: string;          // base-asset amount, decimal string
  price?: string;            // reference price (required for MARKET BUY balance check)
}): Promise<any> {
  const r = await postJson<{ ok: boolean; data: any }>('/api/nia/orders', o);
  return r.data;
}
