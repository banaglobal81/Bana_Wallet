...
web/src/app/[locale]/(site)/  — authenticated user shell. activity/, compensation/, deposit/, portfolio/, referral/, settings/ (+settings/security/), staking/, swap/, wallet/, withdraw/
...
web/src/i18n/              — next-intl config: routing.ts (locale list/defaults), navigation.ts (typed Link/router), request.ts, localeCookie.ts (locale persistence). Owned by `web-shared-expert` (shared infra, like auth/middleware).
