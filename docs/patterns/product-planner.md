...
## "Render nothing for this alert stage" is scoped to the surfaces the deciding doc actually enumerated

A-7 §3.4 ruled `T1_WARNING` invisible to users, and AC-A7-08 froze that as "T1 renders identically to CLEAR". The reasoning was X-3′'s "T1 does not stop user functions" — true for the surfaces A-7 covered (balance, withdrawal, queries). But `assertExecutionAllowed(coin,'NEW_POSITION')` treats a new position as new *issuance* and throws on T1 without an admin override (`coinAuthority.ts:183`), and there is no override on a user route. Inheriting the invisibility rule onto the staking surface therefore ships an enabled button that always 403s. Before carrying a "show nothing" rule to a new surface, read the gate function the new surface calls and check its predicate is the same one the original ruling was reasoning about — and when it is not, narrow the old AC explicitly in the new doc rather than adding a contradicting one. Also collapse *all* of that gate's block reasons into one user string (T2, T1-no-override, transition-in-progress, direct-change-in-progress): the user cannot act on the difference, and splitting them leaks the operational state.

## "You have nothing" is at least two screens: unreachable, and resolves-with-time

`available < minimum` looks like one empty state and is not. If holds are zero and no rail can add funds, the honest sentence is "there is no way to add this" — a non-button chip. If the shortfall is caused by the user's own holds (staking lock, pending withdrawal), the same sentence is a **lie**: the money returns at maturity. Rank the hold-based state above the no-funds state in the precedence list, because that ordering is the only thing preventing the false sentence from rendering. And give the "there is no other way" sentence an expiry guard: render it only while the rails that would falsify it are provably off (`localDepositRail === 'UNSUPPORTED' && yieldRail !== 'CLAIM_LIVE'`), with an AC that flips a rail in a fixture and asserts the sentence disappears — otherwise the day a deposit rail opens, the most confident sentence on the screen becomes the most wrong one. Worked form: `docs/specs/staking-yield-system-v2-design-t8-stake-flow-frd.md` §3.2 / §5.2.

## Deferring an engine is only "deferred" if the commit screen states the absence as a contract term

When the execution engine behind a shipped affordance is postponed, hiding the control is right — a disabled toggle reads as "off for now", i.e. "I can turn it on later", which is the same false promise a disabled button makes. But hiding *alone* converts a known gap into an unstated one: the user still signs a contract whose maturity behaviour changed. Pair the removal with one mandatory, non-collapsible sentence on the **commit** screen (not a settings page, not a tooltip, same locale pass as the other commit disclosures) saying what will not happen. That sentence is the entire difference between "deferred" and "silently degraded", and it is the thing to write first. Four riders:

- **Gate the sentence on the same switch that will falsify it** (a server-surfaced rail flag derived from the one code constant, never a second constant in the client), with an AC that flips the fixture and asserts the sentence *disappears*, plus a fail-closed default when the field is absent — so the screen team is not blocked on the server change landing first.
- **Do not reuse the "not available yet" phrasing from a neighbouring sentence whose feature *is* scheduled.** Two adjacent disclosures with different certainty about the future must read differently, and the translation brief has to say so explicitly — otherwise a translator or the next dev harmonizes them and re-promises the unscheduled one.
- **Strip the option from the request body too, not just the control.** A checkbox that no longer renders while its argument still threads through `onSubmit` → api helper → route is one restored line away from re-promising, and the route rejecting it (rather than ignoring it) is what keeps a stale client from silently signing the old contract.
- **A ruling's wording is policy, not copy.** If the deciding doc says "the principal returns to your wallet" while your own lexicon bans "wallet" (it is not a wallet) and bans "returns" (nothing ever left), re-express the *meaning* in the established vocabulary and disclose the substitution as an escalation row — importing the words instead reintroduces the exact misreadings the lexicon was built to stop.

Worked form: `docs/specs/staking-yield-system-v2-design-t8-stake-flow-frd.md` §6.2 ⓕ / §6.4 / §6.5.
