...
## Invocation Scope
- **If the caller names a specific file/area** (one agent file, one architecture doc, README), read/edit only that file + `CLAUDE.md` — do not re-read the other agent files or unrelated docs for a routine, targeted fix.
...
- Sync items:
  ...
  - Agent count: CLAUDE.md team table row count vs actual files in `.claude/agents/`
  - **Model tiers**: CLAUDE.md's "Agent Team" table `model` column must match each agent file's frontmatter `model:` field exactly. Per CLAUDE.md rule 9, only `pm`/`product-planner`/`game-planner` may be `opus` — flag any other agent set to `opus` as drift, don't just silently accept it.
