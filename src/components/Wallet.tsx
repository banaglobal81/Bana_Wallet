...
export default function Wallet({ settings, onNavigate }: WalletProps) {
  ...
  const loadAdmin = async () => {
    ...
    try {
      ...
      setSettlements(h);
      setAdmState('ok');
    } catch { setAdmState('error'); }
  };

  useEffect(() => { loadUser(); loadAdmin(); }, []);

  const unsettledFees: Array<[string, any]> = unsettled?.unsettledFees && typeof unsettled.unsettledFees === 'object'
    ? Object.entries(unsettled.unsettledFees) : [];
  ...
  return (
    <div className="flex-1 min-h-full bg-[#06132a] text-[#d8e2ff] p-4 sm:p-6 lg:p-8 flex flex-col gap-6 overflow-y-auto">
        ...
        <div>
          ...
          <p className="text-xs sm:text-sm text-[#8c90a0] mt-1 font-mono">
            {view === 'user'
              ...
              : 'Broker / Admin — commissions, settlements, and tenant overview.'}
          </p>
        </div>
      ...
      {/* Admin / User toggle */}
      <div className="flex bg-[#020d24]/60 p-1 rounded-xl border border-[#1E3559] self-start">
        <button
          onClick={() => setView('user')}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
            view === 'user' ? 'bg-[#2E7DFF] text-white shadow' : 'text-[#8c90a0] hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
            view === 'admin' ? 'bg-[#2E7DFF] text-white shadow' : 'text-[#8c90a0] hover:text-white'
          }`}
        >
          <Building2 className="h-3.5 w-3.5" /> Admin (Broker)
        </button>
      </div>
        ...
        <>
          ...
          <div className="p-6 rounded-2xl bg-[#112643]/70 border border-[#1E3559] flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <h3 className="font-sans font-extrabold text-[#d8e2ff] text-sm uppercase tracking-wider flex items-center gap-2">
                <Building2 className="h-4 w-4 text-[#528dff]" /> Broker / Tenant
              </h3>
              <button onClick={loadAdmin} aria-label="Refresh" className="p-2 bg-[#020d24]/60 hover:bg-[#1e3459] border border-[#1E3559] rounded-lg text-[#8c90a0] hover:text-white transition-colors cursor-pointer">
            ...
            <div className="flex items-center justify-between gap-3 p-3.5 bg-[#020d24]/60 border border-[#1E3559] rounded-xl">
              <span className="text-[11px] font-mono text-[#8c90a0]">Tenant ID</span>
              <span className="text-xs font-mono text-[#afc6ff] truncate">{unsettled?.tenantId || settings.connectedWallet.slice(0, 18) + '…'}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-5 rounded-2xl bg-[#112643]/70 border border-[#1E3559] flex flex-col gap-1">
              <span className="text-[11px] font-mono uppercase tracking-widest text-[#8c90a0] font-bold flex items-center gap-1.5">
                <Coins className="h-3.5 w-3.5 text-amber-400" /> Unsettled Commission
              </span>
              {admState === 'loading' ? (
                <span className="text-sm text-[#8c90a0] mt-1">Loading…</span>
              ) : unsettledFees.length === 0 ? (
                <span className="text-2xl font-bold font-sans text-white mt-1">0.00 <span className="text-sm text-[#8c90a0]">USDT</span></span>
              ) : (
                <div className="mt-1 flex flex-col gap-0.5">
                  {unsettledFees.map(([cur, amt]) => (
                    <span key={cur} className="text-lg font-bold font-sans text-white">{String(amt)} <span className="text-xs text-[#8c90a0]">{cur}</span></span>
                  ))}
                </div>
              )}
            </div>
            <div className="p-5 rounded-2xl bg-[#112643]/70 border border-[#1E3559] flex flex-col gap-1">
              <span className="text-[11px] font-mono uppercase tracking-widest text-[#8c90a0] font-bold flex items-center gap-1.5">
                <Receipt className="h-3.5 w-3.5 text-[#528dff]" /> Trade Count
              </span>
              <span className="text-2xl font-bold font-sans text-white mt-1">{unsettled?.tradeCount ?? 0}</span>
            </div>
          </div>

          {/* Settlement history */}
          <div className="p-6 rounded-2xl bg-[#112643]/70 border border-[#1E3559] flex flex-col gap-4">
            <h3 className="font-sans font-extrabold text-[#d8e2ff] text-sm uppercase tracking-wider">Settlement History</h3>
            {admState === 'loading' ? (
              <p className="text-xs font-mono text-[#8c90a0] py-6 text-center">Loading…</p>
            ) : settlements.length === 0 ? (
              <div className="py-8 text-center flex flex-col items-center gap-2">
                <div className="p-3 bg-[#020d24]/60 rounded-full border border-[#1E3559]"><Receipt className="h-6 w-6 text-[#8c90a0]" /></div>
                <p className="text-sm font-bold text-white">No settlements yet</p>
                <p className="text-xs text-[#8c90a0] max-w-xs">Commission settlements between the hub and your broker account will appear here.</p>
              </div>
            ) : (
                ...
                {settlements.slice(0, 8).map((s, i) => (
                  <div key={s.id || i} className="flex items-center justify-between py-2.5 gap-3">
                    <div className="min-w-0">
                      <div className="text-sm font-bold text-emerald-400 font-mono">{s.amount} {s.currency}</div>
                      <div className="text-[10px] font-mono text-[#8c90a0] truncate">{s.settledAt}</div>
                    </div>
                    <span className="shrink-0 text-[9px] font-mono font-bold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">SETTLED</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/15 flex items-start gap-3">
            ...
            <p className="text-xs text-[#8c90a0] leading-relaxed">
              As the <span className="text-slate-200 font-bold">admin / broker</span>, you manage users and collect commissions.
              End-user wallets and funding are created on the Nia-Hub side; this panel reflects your tenant's
              real settlement data via the API key.
            </p>
          </div>
        </>
      )}
